...
The script handles everything: frontend worktree, backend worktree (with compose override patching for SSL certs, `SYTEX_ANGULAR_FRONT_URL`, and `SYTEX_DATABASE_NAME`), database cloning, starting backend containers, and launching `ng serve`.

It outputs the URLs and database name at the end — **always report these to the user**.

### Environment setup notes

Each worktree gets its own MySQL database (`sytex_{branch_slug}`) cloned from the source database via `clone-db.sh`. The compose override is patched with `SYTEX_DATABASE_NAME` pointing to this clone.
...
To destroy a worktree and its database when no longer needed:

```bash
/home/ubuntu/projects/.claude/skills/issue/destroy-worktree.sh {branch-name}
```

The script kills ng serve, runs `just wt-clean` for both front and back, and drops the cloned database.
...
- **Each worktree has its own database** — cloned from master's database via `clone-db.sh`, dropped on cleanup
- **Backend worktrees** have their own Docker services (sytex + proxy), sharing master's infrastructure (RabbitMQ, Redis, Elasticsearch)
