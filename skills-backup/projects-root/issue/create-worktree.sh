...
# ===== Step C — Patch compose override (before starting containers) =====
echo ""
echo "===== Step C: Patch compose override ====="
cd "$ROOT"

FRONT_PORT=$("$FRONT_MASTER/bin/wt-ports.sh" "$BRANCH")
read -r BACK_HTTP BACK_HTTPS < <("$BACK_MASTER/bin/wt-ports.sh" "$BRANCH")

# C.1 Proxy SSL: point volumes to master's certs
...
# C.2 SYTEX_ANGULAR_FRONT_URL → frontend worktree (with trailing slash)
...
# C.3 Patch frontend apiUrl to point to backend worktree
...
echo ""
echo "===== Step D: Start services ====="

# D.1 Backend containers
